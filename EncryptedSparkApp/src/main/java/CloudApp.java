...
public class CloudApp {
    ...
    public static void main(String[] args){
        ...
        String token, vaultAddress;
		System.out.println("Enter vault address.(0 for default)");
		vaultAddress = scanner.nextLine();
		if(vaultAddress.equals("0"))
			vaultAddress = "http://127.0.0.1:8200/";
        System.out.println("Enter user token.");
        token = scanner.nextLine();
		spark.sparkContext().hadoopConfiguration().set("parquet.vault.address", vaultAddress);
        spark.sparkContext().hadoopConfiguration().set("parquet.decryption.user.token", token);
        String path;
        System.out.println("Enter path for People file.");
        ...
        /*userDf = spark.read()
                ...
                .csv(path);*/
        userDf = spark.read().parquet(path);
        userDf.createOrReplaceTempView("userView");
        System.out.println("Enter path for Vehicles file.");
        path = scanner.nextLine();
        /*vehicleDf = spark.read()
                ...
                .option("inferSchema", "true")
                .csv(path);*/
        vehicleDf = spark.read().parquet(path);
        vehicleDf.createOrReplaceTempView("vehicleView");
        System.out.println("Enter path for Events file.");
        path = scanner.nextLine();
        /*eventDf = spark.read()
                .option("sep", "\t")
                .option("header", "true")
                .option("inferSchema", "true")
                .csv(path);*/
        eventDf = spark.read().parquet(path);
