...
public class ParquetOutputFormat<T> extends FileOutputFormat<Void, T> {
  ...
  public RecordWriter<Void, T> getRecordWriter(TaskAttemptContext taskAttemptContext)
    ...
    String userToken = conf.getTrimmed("parquet.encryption.user.token");
    ...
    if (encryptionKeyId != null && userToken != null) { //Throw if one of them is null?
      ...
      if(keyBytes == null){
          throw new IOException("No such key available. (" + encryptionKeyId + ")");
      }
      eSetup = new EncryptionSetup(ParquetEncryptionFactory.PARQUET_AES_GCM_CTR_V1, keyBytes, keyId);
      //System.out.println("Parquet key id: "+ keyId);
      String extension = codec.getExtension() + ".parquet"+encryptedSuffix;
      //System.out.println("ext: "+extension);
      file = getDefaultWorkFile(taskAttemptContext, extension);
      //TODO file is without .encrypted ext!!
      ...
      if(null != columnsToEncrypt && columnsToEncrypt.length > 0) {
        ...
        for(int i = 0; i < columnsToEncrypt.length; i++) {
          ColumnMetadata cmd = new ColumnMetadata(true, columnsToEncrypt[i]);
          columns.add(cmd);
        }
      ...
      //System.out.println("Creating encrypted Parquet file: " + file);
    }else{
  ...
  public RecordWriter<Void, T> getRecordWriter(Configuration conf, Path file, CompressionCodecName codec,
    ...
    ParquetFileWriter w = new ParquetFileWriter(
        conf, init.getSchema(), file, Mode.CREATE, blockSize, maxPaddingSize, fileEncryptor);
    w.start();
